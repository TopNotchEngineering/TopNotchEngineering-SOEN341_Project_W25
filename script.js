...
describe('showLoginForm', () => {
  ...
  beforeEach(() => {
    ...
    global.document = document; // Make the document available globally
  });

  test('displays admin login form', () => {
    showLoginForm(1);

    expect(document.getElementById('loginSection').classList.contains('hidden')).toBe(false);
    expect(document.getElementById('loginTitle').innerText).toBe("Admin Login");
    expect(document.getElementById('role').value).toBe("1");
  });

  test('displays user login form', () => {
    showLoginForm(0);

    expect(document.getElementById('loginSection').classList.contains('hidden')).toBe(false);
    expect(document.getElementById('loginTitle').innerText).toBe("User Login");
    expect(document.getElementById('role').value).toBe("0");
  });
});